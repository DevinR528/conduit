            ...
            loop {
                select! {
                    Some(server) = futures.next() => {
                        ...
                        match server {
                            Ok((server, _response)) => waiting_servers.remove(&server),
                            Err((server, _e)) => waiting_servers.remove(&server),
                        };
                    },
                    Some(event) = &mut subscriber => {
                        if let sled::Event::Insert { key, .. } = event {
                            ...
                            if let Some((server, pdu_id)) = utils::string_from_bytes(
                                ...
                                .ok()
                                .filter(|server| waiting_servers.get(server).is_none())
                                .and_then(|server| parts
                                .next()
                                .ok_or_else(|| Error::bad_database("Invalid serverpduid in db.")).ok().map(|pdu_id| (server, pdu_id)))
                            {
                                waiting_servers.insert(server.clone());
                                futures.push(Self::handle_event(server.clone(), pdu_id.into(), &globals, &rooms));
                            }
                        }
                    }
                }
